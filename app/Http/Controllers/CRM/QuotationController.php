...
//    }
    ...
    public function createCart(Request $request, $id)
    {
        ...
        $quotation->subject = $request->subject;
        $quotation->body = $request->body;
        $quotation->totalAmount = $request->subtotal;
        $quotation->totalVat = $request->vatC;
        $quotation->grandTotal = $request->total;
//        $quotation->save();
        $cart = new HouseAreaCart();
        $cart->quotationId = $quotation->id;
        $houseAreas = $request->houseAreaTypeId;
//        Log::info($houseAreas);
        $ht= [];
        foreach ($houseAreas as $key=>$value){
            Log::info($value['id']);
        }
        $cart->houseAreaCartInfo = json_encode($ht);
//        $cart->save();
